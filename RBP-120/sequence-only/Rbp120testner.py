...
from DeepFusion import DeepFusion
from Rbp120DataLoader import RbpDataset
...
class RbpTestner():
    ...
    def test(self, name):
        ...
        test_dataset = RbpDataset('RBP-120/sequence-only/sample/'+name, name, 'test', False)
...
if __name__=='__main__':
    rbplist = glob('RBP-120/sequence-only/sample/*')
    print(rbplist) 
    log_path = 'RBP-120/sequence-only/results/' + time.strftime("%m-%d-%H", time.localtime())
    ...
    for rbpname in rbpnames:
        if '.py' in rbpname:
            continue
        model_dir = 'RBP-120/sequence-only/models/' + rbpname + '.best*'
