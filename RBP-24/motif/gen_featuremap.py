...
class RbpTestner():
    ...
    def test(self, name):
        ...
        model = model.to('cuda')
        model.eval()
        train_dataset = RbpDataset('../sample/{}/{}.train_1.txt'.format(name, name))
        train_dataloader = DataLoader(train_dataset, batch_size=1, shuffle=False, num_workers=0)
        features = []
        for i in range(16):
            features.append([])
        for idx, (motif, seq) in enumerate(train_dataloader):
            seq = seq[0]
            motif = motif.to('cuda')
                    ...
                    for k in range(logit.shape[0]):
                        if logit[k] > 0:
                            ...
                            if 'N' not in seq[posi : posi+10]:
                                features[j].append((seq[posi : posi+10].replace('T', 'U'), logit[posi], idx, j))
...
if __name__=='__main__':
    ...
    for rbpname in rbpnames:
        ...
        rbp_feature = testner.test(rbpname)